...
use crate::{
    lib::{MilestoneEscrowContract, MilestoneEscrowContractClient},
    types::{EscrowStatus, MilestoneInput, MilestoneStatus},
};
use soroban_sdk::{
...
fn test_create_and_approve_milestones() {
    ...
    assert_eq!(escrow_sub.milestones.get(0).unwrap().status, MilestoneStatus::Submitted);
    ...
    // Contractor submits and client approves milestone 1
...
fn test_dispute_and_resolve_flow() {
    ...
    client.dispute_milestone(&escrow_id, &0, &reason);
